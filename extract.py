...
def filter_valid_dumps(
    source_schema: Dict[str, Any], run_date: datetime, end_date: datetime
) -> List[Dict[str, Any]]:
    ...
    for weekly_dump in source_schema["db_dumps"]:
        valid_dump = (
            weekly_dump.get("start_date") >= run_date
            and weekly_dump.get("start_date") < end_date
            and weekly_dump.get("num_matches") != 0
        )
        if valid_dump:
...
def download_data(
    downloader: ApiDownloader, valid_dumps: List[Dict[str, Any]], data_dir: str = "data"
):
