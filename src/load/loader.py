...
def load_json_data(model, file_dir, fn, bucket, s3):
    """Loads Pydantic model as json files in S3."""
    file_name = f"{file_dir}/{fn}.json"
...
def load_parquet_data(data, file_dir, fn, bucket, s3):
    """Loads parquet data directly into S3 bucket."""
    file_name = f"{file_dir}/{fn}.parquet"
